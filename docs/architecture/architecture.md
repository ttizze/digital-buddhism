...
トップの `/$locale/tipitaka` は一覧取得のPromiseをloaderから返し、TanStack Routerの
`Await`で解決します。一覧の取得中はヘッダー・見出しとツリー形状のスケルトンを
先に表示し、取得後の一覧をSSRでストリーミングします。メタデータは一覧を待たずに
生成します。一覧取得自体の処理時間やデータ量を減らす変更ではありません。

記事の `/$locale/tipitaka/$pageSlug` は存在確認・翻訳タイトル・説明・翻訳済み言語を
先に確定し、本文とナビゲーションをPromiseで返します。ルート翻訳の取得は並行して
開始し、メタデータの待ち合わせから外します。`Await`と`pendingComponent`は同じ
shadcn `Skeleton`を使い、本文到着後に記事へ置き換えます。存在しない記事はloaderで
404を返し、本文取得の失敗はエラー境界へ伝えます。Markdown出力は本文の解決を待ちます。
...
記事の翻訳は自ページ・root・homeのoverlayから取得する。祖先と子孫の題名は
homeから適用し、子孫ごとのpointerは読まない。通常はbase/stateを含めて最大8回の
KV getとなる。同一ページとrootは重複取得しない。KV伝播中に前revisionへ戻る場合は
該当overlayごとに1回増える。

選択中の語義と得票はページの翻訳overlayへ保存し、本文の文字列payloadへ含める。
本人の票は公開snapshotへ保存しない。匿名表示は語義APIを呼ばず、ログイン後に
選択語義の得票と本人の投票状態だけを取得する。クライアントのキャッシュキーは
ページ・言語・ユーザーIDで分け、ログアウト後は公開値へ戻す。

記事の子ページ一覧・ページツリー・目次は、閉じた枝の子を描画しない。
一覧の直下リンクと現在ページへの展開済み経路は初期HTMLに残るが、閉じた階層の
子孫リンクは展開するまでHTMLに含まれない。各親ページの直下リンクとsitemapが
探索経路となる。共有ダイアログは開くときに読み込み、ロケールメッセージは
`$locale`のserver functionが対象言語分だけ返す。
