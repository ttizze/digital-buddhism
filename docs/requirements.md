...
- 本文記事の境界とタイトルはTipitaka.org用 `*.toc.xml` に合わせる
- 本文ページは `MULA` / `ATTHAKATHA` / `TIKA` / `OTHER` のテキスト階層を持つ
- ルート、カテゴリ、複数記事を束ねる原典目次ページはテキスト階層を持たず、親子構造（`parentId`）と並び順（`position`）だけを持つ
...
- 選択語義と得票はKVから初期表示し、ログイン時だけ本人の投票状態を追加取得する
...
- Tipitaka本文の分割位置を変更しても、一致するセグメントの翻訳・投票・語義を保持し、対応不能な投稿データは削除しない
- 翻訳投票は翻訳ID + ユーザーIDで一意
