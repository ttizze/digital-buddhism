...
### 原文の単語訳
- 単語訳（内部名: gloss）は、原文の単語または語句に付ける短い翻訳とする
- 原文はセグメントごとの単語に分割し、各単語は開始・終了位置、原語表記、並び順を保持する
- 語義は単語とロケールに紐づく候補として複数登録できる
- 表示する語義は、明示的な採用候補を優先し、採用候補がなければ point、作成日時、ID の降順で選ぶ
- 原文と訳文の既存の書体・色・表示モードを維持し、語義がある範囲だけを分割して原文の上に表示する
- 単語訳をクリックすると、モーダルを使わず本文直下へ、翻訳と共通の候補一覧、追加、本人削除、投票UIを表示する
- UIでは単語訳も Translation として扱い、独自の「語義」ラベルを表示しない
- 投票により語義候補の point を更新し、現在ユーザーの投票状態を返す
...
- 翻訳投票は翻訳ID + ユーザーIDで一意
- 単語はセグメント + 並び順、およびセグメント + 開始位置 + 終了位置で一意
- 語義投票は語義候補ID + ユーザーIDで一意
- 採用語義は単語 + ロケールで一意
