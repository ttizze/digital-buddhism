...
import { calcProofStatus } from "../_domain/translation-proof-status";
...
async function updateProofStatus(
	...
	const stats = await tx
		...
		.select([
			sql<number>`cast(count(distinct segments.id) as integer)`.as(
				"totalSegments",
			),
			sql<number>`cast(count(distinct case when segment_translations.point >= 1 then segments.id end) as integer)`.as(
				"segmentsWith1PlusVotes",
			),
			sql<number>`cast(count(distinct case when segment_translations.point >= 2 then segments.id end) as integer)`.as(
