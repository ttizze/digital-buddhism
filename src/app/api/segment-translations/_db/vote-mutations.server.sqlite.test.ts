...
import { handleVote } from "./vote-mutations.server";

describe("handleVoteのSQLite互換性", () => {
	it("翻訳数に影響されずセグメント単位でproofを集計する", async () => {
			...
			await runWithDatabaseRequestContext(
				...
				async () => {
					...
					await client.execute(
						"CREATE TABLE tipitaka_pages (id INTEGER PRIMARY KEY)",
					);
					await client.execute(
						"CREATE TABLE segments (id INTEGER PRIMARY KEY, tipitaka_page_id INTEGER NOT NULL)",
					);
					await client.execute(
					...
					await client.execute("INSERT INTO tipitaka_pages (id) VALUES (1)");
					await client.execute(
						"INSERT INTO segments (id, tipitaka_page_id) VALUES (1, 1), (2, 1)",
					);
					await client.execute(
						"INSERT INTO segment_translations (id, segment_id, locale, point, user_id) VALUES (1, 1, 'ja', 0, 'translator'), (2, 1, 'ja', 0, 'translator-2'), (3, 2, 'ja', 1, 'translator')",
					);
